...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      // Insert directly into Supabase to avoid circular dependency
      const { data, error } = await supabaseAdmin
        .from('healing_products')
        .insert({
          product_title: req.body.product_title,
          product_purpose: req.body.product_purpose || null,
          how_to_use: req.body.how_to_use || null,
          product_image_path: req.body.product_image_path || null,
          amazon_url: req.body.amazon_url || null,
          is_active: req.body.is_active ?? true,
          product_order: req.body.product_order || null,
      ...
      if (error) {
        ...
        return res.status(500).json({ error: 'Failed to create healing product' });
      }
