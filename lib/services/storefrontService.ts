...
class StorefrontService {
  ...
  getAll(): StorefrontProduct[] { const raw = typeof localStorage!=='undefined' ? localStorage.getItem(this.STORAGE_KEY):null; if(!raw) return []; try { const p: StorefrontProduct[] = JSON.parse(raw); return Array.isArray(p) ? p : []; } catch { return []; } }
  getPublished(): StorefrontProduct[] { return this.getAll().filter(p=>p.status==='published'); }
  getFavorites(): StorefrontProduct[] { return this.getPublished().filter(p=>p.showInFavorites).sort((a,b)=>(a.sortWeight??0)-(b.sortWeight??0)); }
  getByCategory(category: StorefrontProduct['category'], opts?: { includeDraft?: boolean }): StorefrontProduct[] { const list=this.getAll().filter(p=>p.category===category && (opts?.includeDraft ? p.status!=='archived' : p.status==='published')).sort((a,b)=>(a.sortWeight??0)-(b.sortWeight??0)); return list; }
  findBySlug(slug: string): StorefrontProduct | undefined { return this.getAll().find(p=>p.slug===slug); }
